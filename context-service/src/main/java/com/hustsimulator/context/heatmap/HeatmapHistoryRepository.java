...
public interface HeatmapHistoryRepository extends JpaRepository<HeatmapHistory, UUID> {
    ...
    @Query(value = "SELECT day_bucket, CAST(MAX(total_count) AS BIGINT) FROM (SELECT date_trunc('hour', h.recorded_at) AS hour_bucket, date_trunc('day', h.recorded_at) AS day_bucket, SUM(h.average_count) AS total_count FROM context.heatmap_history h WHERE h.recorded_at >= :since GROUP BY hour_bucket, day_bucket) sub GROUP BY day_bucket ORDER BY day_bucket", nativeQuery = true)
    List<Object[]> findDailyDensitySince(@org.springframework.data.repository.query.Param("since") LocalDateTime since);

    @Query(value = "SELECT month_bucket, CAST(MAX(total_count) AS BIGINT) FROM (SELECT date_trunc('hour', h.recorded_at) AS hour_bucket, date_trunc('month', h.recorded_at) AS month_bucket, SUM(h.average_count) AS total_count FROM context.heatmap_history h WHERE h.recorded_at >= :since GROUP BY hour_bucket, month_bucket) sub GROUP BY month_bucket ORDER BY month_bucket", nativeQuery = true)
